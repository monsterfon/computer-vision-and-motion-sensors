...
total_distance = sum(sqrt(sum(diff(s_raw).^2, 2)));
fprintf('Total distance traveled without -g: %.4f m\n', total_distance);
...
% Po prepoznanem začetku gibanja začnite računati kot in os rotacije ter rotacijsko matriko
...
subplot(3,1,1);
plot(t, a_ref);
title('Transformed Acceleration Data');
xlabel('Time (s)');
ylabel('Acceleration (m/s^2)');
legend('X', 'Y', 'Z');
...
for i = 1:3
    p_drift = polyfit([st1 st2], [v(st1,i); v(st2,i)], st_polinoma);  
    v_drift(st1:end,i) = polyval(p_drift, st1:length(v));
end
...
subplot(5,1,1);
plot(t, acc);
title('Compensated Accelerometer Data');
xlabel('Time (s)');
ylabel('Acceleration (m/s^2)');
legend('X', 'Y', 'Z');
...
subplot(5,1,2);
plot(t, om_corrected);
title('Compensated Gyroscope Data');
xlabel('Time (s)');
ylabel('Angular Velocity (rad/s)');
legend('X', 'Y', 'Z');
